...
## VSP Inference on the Example Checkpoint

After the two training stages, you can run FrozenLake visual spatial planning inference with the example checkpoint:

```bash
uv run python script/run_interleaved_vsp_example.py
```

By default, this script uses:

- model checkpoint: `/inspire/hdd/project/qproject-fundationmodel/yangyi-253108120173/jjc/ssd/codebase/LatentUM/outputs/interleaved-vision/checkpoint-10`
- decoder checkpoint: `ckpt/pixel_decoder`
- input image: `asset/frozenlake_interleaved_example/images/0.png`
- output directory: `asset/frozenlake_interleaved_vsp_demo`

It will save decoded step images, write an `inference_log.json`, and produce a GIF when multiple frames are generated.

## Notes

- This is a teaching example, not a full training framework.
- The scripts expect a pretrained LatentUM checkpoint that already includes tokenizer, InternVL backbone, quantizer, and AR head weights.
- At minimum, run `uv run python -m compileall model script` after modifications.
